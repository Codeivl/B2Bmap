class RegionMap {
  ...
  hasTile(tileX, tileZ, unminedZoomLevel) {
    ...
    const maxTileX =
      Math.ceil(
        ((this.worldMinX + this.worldWidth) * zoomFactor) / this.tileSize
      ) - 1;
    const maxTileZ =
      Math.ceil(
        ((this.worldMinZ + this.worldHeight) * zoomFactor) / this.tileSize
      ) - 1;
...
class Unmined {
  ...
  createLegend() {
    ...
    this.#legendContainer.className = "unmined-legend";
    this.olMap.getTargetElement().appendChild(this.#legendContainer);

    this.#legendButton = document.createElement("button");
    this.#legendButton.className = "unmined-legend-button";
    this.#legendButton.textContent = "Show Legend";
    this.#legendButton.addEventListener("click", () => this.toggleLegend());
    this.olMap.getTargetElement().appendChild(this.#legendButton);
  ...
  updateLegend() {
    ...
    const legendItems = this.#options.markers.filter((marker) => marker.legend);
    ...
    legendItems.forEach((item) => {
      ...
      legendItem.className = "unmined-legend-item";
      ...
      text.textContent = item.legendLabel || item.text; // Use legendLabel if available, otherwise fallback to text
  ...
  toggleLegend() {
    this.#legendContainer.classList.toggle("unmined-legend-visible");
    if (this.#legendContainer.classList.contains("unmined-legend-visible")) {
      this.#legendButton.textContent = "Hide Legend";
      this.adjustLegendSize();
    } else {
      this.#legendButton.textContent = "Show Legend";
    }
  }
  ...
  updateLegendVisibility() {
    ...
    this.#legendContainer.classList.toggle("unmined-legend-mobile", isMobile);

    if (isMobile) {
      this.#legendButton.style.display = "block";
      this.#legendContainer.classList.remove("unmined-legend-visible");
    } else {
      this.#legendButton.style.display = "none";
      this.#legendContainer.classList.add("unmined-legend-visible");
    }
  ...
  adjustLegendSize() {
    const isMobile = window.innerWidth <= 768;
    if (isMobile) {
      const legendItems = this.#legendContainer.querySelectorAll(
        ".unmined-legend-item"
      );
      const itemHeight =
        legendItems.length > 0 ? legendItems[0].offsetHeight : 0;
      const padding = 20; // Account for padding
      const totalHeight = itemHeight * legendItems.length + padding;

      // Set a minimum height to prevent tiny legends
      const minHeight = 100;
      const maxHeight = window.innerHeight * 0.8;

      this.#legendContainer.style.height = `${Math.min(
        Math.max(totalHeight, minHeight),
        maxHeight
      )}px`;
      this.#legendContainer.style.overflowY =
        totalHeight > maxHeight ? "auto" : "visible";
    } else {
      this.#legendContainer.style.height = "auto";
      this.#legendContainer.style.overflowY = "visible";
    }
  }

  createTooltip() {
    this.#tooltip = document.createElement("div");
    this.#tooltip.className = "unmined-tooltip";
    this.#tooltip.style.display = "none";
    this.olMap.getViewport().appendChild(this.#tooltip);
  }

  addMarkerInteractions() {
    const interactionTypes = ["click", "pointermove"];

    interactionTypes.forEach((interactionType) => {
      this.olMap.on(interactionType, (evt) => {
        if (evt.dragging) {
          return;
        }
        ...
        if (feature && feature.get("isMarker")) {
          ...
          this.showTooltip(feature, coordinate);
        } else if (interactionType === "pointermove") {
          this.hideTooltip();
        }
      });
    });
  ...
  showTooltip(feature, coordinate) {
    if (this.#currentTooltipFeature !== feature) {
      const markerData = feature.get("markerData");
      if (markerData && markerData.desc) {
        this.#tooltip.innerHTML = `<span>${markerData.desc}</span>`;
        this.#tooltip.style.display = "block";
        this.olMap.getOverlayContainer().appendChild(this.#tooltip);

        const iconSize = markerData.imageScale * 32; // Assuming 32px icon size
        const tooltipHeight = this.#tooltip.offsetHeight;

        // Calculate the tooltip position based on the marker's position and size
        const markerPosition = feature.getGeometry().getCoordinates();
        const markerPixel = this.olMap.getPixelFromCoordinate(markerPosition);
        const markerWidth = iconSize * markerData.imageAnchor[0] * 2;
        const markerHeight = iconSize * markerData.imageAnchor[1];

        const tooltipX = markerPixel[0] - this.#tooltip.offsetWidth / 2;
        const tooltipY = markerPixel[1] - tooltipHeight - markerHeight - 30;

        this.#tooltip.style.left = `${tooltipX}px`;
        this.#tooltip.style.top = `${tooltipY}px`;

        this.#currentTooltipFeature = feature;
      }
    }
  }

  hideTooltip() {
    if (this.#currentTooltipFeature) {
      this.#tooltip.style.display = "none";
      this.#currentTooltipFeature = null;
    }
  }
